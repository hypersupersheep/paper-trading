...
# 节点侧回执 v6 补充 —— `lan_ip()` 挑物理网卡(你的 v7 补充)app v1.12.3

收到你 v7 补充:装了 Docker/VM/VPN 的机器,`lan_ip()`(connect 8.8.8.8 读 getsockname)挑到**虚拟网卡**(你那台是 Docker 桥 `172.19.0.1`)→ 登记 `base_url` 外部不可达 → Admin timeout。**已修。**

## 改法
`lan_ip()` 不再只信「8.8.8.8 默认出口」(Mac+Docker 上它就是错的),改成**多源枚举 + 按网段挑**:
- 枚举源:默认出口(`8.8.8.8` trick,Linux 上常对)+ `gethostbyname_ex` + `getaddrinfo`(Mac/Win 上常能列出真实 IP)。
- 排序优先级:**`192.168.*`(最优)> `10.*` > 其它物理段 > 容器/VPN/链路本地段 > 回环**。
  - **跳过/降权**:`172.16/12`(Docker 桥)、`100.64/10`(Tailscale CGNAT)、`169.254`(链路本地)、`127.`(回环)——仅当没有更好的才兜底用。
- 实测我这台(同时有 Docker `172.19` / Parallels `10.211`·`10.37` / WiFi `192.168.0.186`):`lan_ip()` 现在稳定返回 **`192.168.0.186`**,登记的 `base_url` 外部可达。
- 这类机器现在**留空「本机地址」也能自动报对**,不用手填了;手填(base_url 覆盖)仍作为终极逃生口保留。

## 一处 app 替不了的(确认你说的)
- **Windows 节点首次监听 0.0.0.0 会弹防火墙「允许」对话框**——需用户点「允许」(要管理员),app 无法自动代点。这是 OS 行为,装机说明里写一句即可。Mac 防火墙默认关,无此问题。

## 目标达成
同事:下载 → 打开 → 数据源页填 Admin 地址+口令 → 保存 → **重启一次**(让绑 0.0.0.0 生效)→ 自动上墙。全程不碰命令行(Windows 多点一次防火墙「允许」)。节点 app **v1.12.3**。

---

# 节点侧回执 v7 —— 内置老板机地址,**零配置直连**(app v1.13.0)

进一步:节点 app 现**内置默认 Admin 地址 `http://192.168.0.58:8800`(老板机)**。新机器**开箱即自动指向、开机即绑 0.0.0.0 + 自注册上墙,同事一步都不用点**(连填地址都省了)。

- 仍可在「Admin 对接」卡改成别的地址(换机用)或**清空=断开**(纯本地)。默认不固化进配置文件,所以将来换老板机、我改默认发版,**没手动配过的同事会自动跟到新地址**。
- **没用 register 共享口令**(你们没设):节点→Admin 登记不带 `X-Admin-Token`;Admin→节点 的 `node.token` 鉴权照常自动开着,节点不裸奔。
